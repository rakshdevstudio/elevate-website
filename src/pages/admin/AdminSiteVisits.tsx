            ...
            {upcomingVisits.map((visit) => (
              <Link key={visit.id} to={`/admin/lead/${visit.lead_id}`} className="block p-3 rounded-xl bg-secondary/15 hover:bg-secondary/25 transition-colors">
                <div className="flex items-center justify-between">
                  ...
                  {visit.lead_phone && (
                    <a
                      href={`https://wa.me/${visit.lead_phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hi ${visit.lead_name}, reminder: your site visit is scheduled for ${new Date(visit.scheduled_date).toLocaleDateString("en-IN", { day: "numeric", month: "long" })}${visit.scheduled_time ? ` at ${visit.scheduled_time}` : ""}. - X Elevators`)}`}
                      target="_blank"
